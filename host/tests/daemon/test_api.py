...
class FakeManager:
    def __init__(self) -> None:
        self.connect_calls: list[tuple[str, int, dict]] = []
    ...
    def connect_board(self, *, board_id: str, transport_type_id: int, transport_overrides: dict):
        self.connect_calls.append((board_id, transport_type_id, dict(transport_overrides)))
        return {
            "board_id": board_id,
    ...
    def describe_board(self, board_id: str):
        return {"board_id": board_id, "status": {"connected": True}}

    def disconnect_board(self, board_id: str):
        return {"board_id": board_id, "disconnected": True}

    def rename_board(self, board_id: str, *, new_board_id: str):
        return {"board_id": new_board_id}

    def refresh_sensors(self, board_id: str):
        return {"board_id": board_id, "sensors": []}

    def get_uptime(self, board_id: str):
        return {"board_id": board_id, "uptime_ms": 123}

    def read_sensor(self, board_id: str, *, sensor_runtime_id: int):
        return {"board_id": board_id, "sensor_runtime_id": sensor_runtime_id, "reading": {}}

    def set_period(self, board_id: str, *, sensor_runtime_id: int, period_ms: int):
        return {"board_id": board_id, "sensor_runtime_id": sensor_runtime_id, "period_ms": period_ms}

    def get_period(self, board_id: str, *, sensor_runtime_id: int):
        return {"board_id": board_id, "sensor_runtime_id": sensor_runtime_id, "period_ms": 1000}

    def start_stream(self, board_id: str, *, sensor_runtime_id: int):
        return {"board_id": board_id, "sensor_runtime_id": sensor_runtime_id, "streaming": True}

    def stop_stream(self, board_id: str, *, sensor_runtime_id: int):
        return {"board_id": board_id, "sensor_runtime_id": sensor_runtime_id, "streaming": False}

    def drain_readings(self, board_id: str, *, sensor_runtime_id: int | None = None, limit: int = 200):
...
def test_connect_by_transport_label_resolves_and_passes_overrides():
    ...
    assert status == 201
    assert payload["ok"] is True
    assert payload["data"]["board_id"] == "b1"
    assert manager.connect_calls == [("b1", 1, {"port": "COM4"})]
